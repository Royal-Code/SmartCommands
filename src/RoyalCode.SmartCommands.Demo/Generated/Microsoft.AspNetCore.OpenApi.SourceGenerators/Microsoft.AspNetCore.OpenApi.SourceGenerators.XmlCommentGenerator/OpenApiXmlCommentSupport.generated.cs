...
overriding the automatic entity detection performed by the source generator.", null, null, null, null, false, null, null, null));
            ...
            cache.Add(@"M:RoyalCode.SmartCommands.IUnitOfWorkAccessor`1.BeginAsync(System.Boolean,System.Threading.CancellationToken)", new XmlComment(@"    Invoked when the unit of work is about to begin.
    A transaction is started when the adapter option enables transactions or when
requireTransaction is true (DF21: the generated handler passes
true for commands annotated with [WithTransaction]). The adapter owns only the
...
public class ProductDetails
{
    ...
    public bool Active { get; set; }
            ...
            cache.Add(@"T:RoyalCode.SmartCommands.MapFindByAttribute`1", new XmlComment(@"    Maps a DTO class to an HTTP GET endpoint that finds (retrieves) a single entity by an
alternate or composite key — one or more direct properties of TEntity — and
projects it into the decorated DTO type. Unlike MapFindAttribute, which finds by the
entity identifier, this attribute finds by named properties.
    This attribute requires a service of IRepositoryAccessor&lt;TEntity&gt; to be available via
...
the entity, and the not-found problem names the entity and lists the search criteria.
    Each property declared in propertyNames (prefer nameof) must be a direct, public and
readable property of TEntity, and must be matched — case-insensitively — by a
route placeholder of the same name; the generator derives the parameter type from the property. The
search combines the criteria with AND, in declaration order (used to build the not-found
...
reported at compile time (RCCMD056).
    You may group the endpoint with MapGroupAttribute, add metadata using
WithDescriptionAttribute, WithSummaryAttribute and
WithTagsAttribute, add endpoint filters with WithEndpointFilterAttribute&lt;TFilter&gt;
and enforce security via WithAuthorizationAttribute or WithPolicyAttribute.
    Endpoint handler code is generated by MapApiHandlersAttribute in a static partial class.
    Example — simple alternate key (SKU):
...
public class ItemPedidoDetalhes { /* ... */ }```", null, null, null, null, false, null, null, null));
            cache.Add(@"M:RoyalCode.SmartCommands.MapFindByAttribute`1.#ctor(System.String,System.String,System.String[])", new XmlComment(@"Maps an endpoint to find (retrieve) a specific entity by an alternate or composite key.", null, null, null, null, false, null, [new XmlParameterComment(@"endpointRoutePattern", @"The route pattern for the endpoint, with a named placeholder for each declared property,
e.g. `""por-sku/{sku}""`. When the class declares MapGroupAttribute, the group
prefix is prepended.", null, false), new XmlParameterComment(@"endpointName", @"A unique endpoint name, e.g. `""produto-por-sku""`.", null, false), new XmlParameterComment(@"propertyNames", @"The names of the direct properties of TEntity that form the key, matched
(case-insensitively) to the named placeholders of the route pattern. Prefer declaring them with
`nameof`. The order is preserved to build the not-found problem.", null, false)], null));
...
next delegate that executes the remainder of the pipeline. There is no shared mutable position,
so invoking next more than once deterministically re-executes the rest of the pipeline, and no
enumerator or other disposable resource is retained.", null, null, null, null, false, null, null, null));
            cache.Add(@"M:RoyalCode.SmartCommands.Mediator`2.#ctor(System.Collections.Generic.IEnumerable{RoyalCode.SmartCommands.IDecorator{`0,`1}},System.Func{System.Threading.Tasks.Task{`1}},`0,System.Threading.CancellationToken)", new XmlComment(@"Initializes a new instance of the Mediator&lt;TModel, TResult&gt; class.", null, null, null, null, false, null, [new XmlParameterComment(@"decorators", @"The decorators to run, in registration order, before the final handler.", null, false), new XmlParameterComment(@"finalHandler", @"The delegate that executes the command logic once all decorators have run.", null, false), new XmlParameterComment(@"model", @"The command instance being handled.", null, false), new XmlParameterComment(@"ct", @"Cancellation token.", null, false)], null));
            cache.Add(@"M:RoyalCode.SmartCommands.Mediator`2.NextAsync", new XmlComment(@"Executes the pipeline: the decorators in registration order and, at the end, the final handler.
Each invocation runs the whole pipeline again.", null, null, @"The result produced by the pipeline.", null, false, null, null, null));
...
of work save is already atomic.
    Requires a unit of work (WithUnitOfWorkAttribute&lt;T&gt;, WithDbContext or
WithWorkContextAttribute); without one, the generator reports RCCMD043. This attribute
is opt-in only: there is no per-command way to disable transactions enabled by the adapter option.", null, null, null, null, false, null, null, null));
...
WithDbContextAttribute.", null, null, null, null, false, null, null, null));
            ...
            cache.Add(@"M:RoyalCode.SmartCommands.WorkContext.Adapters.UnitOfWorkAccessor`1.BeginAsync(System.Boolean,System.Threading.CancellationToken)", new XmlComment(null, null, null, null, null, false, null, null, null));
            cache.Add(@"M:RoyalCode.SmartCommands.WorkContext.Adapters.UnitOfWorkAccessor`1.CompleteAsync(System.Threading.CancellationToken)", new XmlComment(@"    Saves the work context and, when the adapter began a transaction, commits it.
    Aligned with DF14: the returned Result represents only success or the problems
...
cleanup and is then rethrown. Commit/rollback failures are no longer converted into the result.", null, null, null, null, false, null, null, null));
            cache.Add(@"M:RoyalCode.SmartCommands.WorkContext.ConcurrencyRetryOperations.DefaultFor``1", new XmlComment(@"The default operation key for the command type TCommand.", null, null, @"The namespace-qualified name of the command type.", null, false, null, null, null));
            cache.Add(@"M:RoyalCode.SmartCommands.WorkContext.ConcurrencyRetryOperations.DefaultFor(System.Type)", new XmlComment(@"The default operation key for the given command type.", null, null, @"The namespace-qualified name of the command type.", null, false, null, [new XmlParameterComment(@"commandType", @"The command type.", null, false)], null));
            ...
            cache.Add(@"M:RoyalCode.SmartCommands.WorkContext.Extensions.CommandsWorkContextExtensions.AddConcurrencyRetryProblem``1(Microsoft.Extensions.DependencyInjection.IServiceCollection,RoyalCode.SmartCommands.WorkContext.ConcurrencyRetryProblemDelegate{``0})", new XmlComment(@"    Registers a retry-exhausted problem factory for a command that uses the default operation key
([WithRetryOnConcurrency] without Operation).
    The default key is string ConcurrencyRetryOperations.DefaultFor&lt;TCommand&gt;() — the
namespace-qualified name of the command type.", null, null, @"The same services for chaining.", null, false, null, [new XmlParameterComment(@"services", @"The service collection.", null, false), new XmlParameterComment(@"factory", @"The problem factory.", null, false)], null));
            cache.Add(@"M:RoyalCode.SmartCommands.WorkContext.Extensions.CommandsWorkContextExtensions.AddConcurrencyRetryProblem``1(Microsoft.Extensions.DependencyInjection.IServiceCollection,RoyalCode.SmartCommands.WorkContext.ConcurrencyRetryProblemServiceDelegate{``0})", new XmlComment(@"    Registers a retry-exhausted problem factory, with access to DI, for a command that uses the default
operation key ([WithRetryOnConcurrency] without Operation).
    The default key is string ConcurrencyRetryOperations.DefaultFor&lt;TCommand&gt;() — the
namespace-qualified name of the command type.", null, null, @"The same services for chaining.", null, false, null, [new XmlParameterComment(@"services", @"The service collection.", null, false), new XmlParameterComment(@"factory", @"The problem factory.", null, false)], null));
...
When `null`, a generic Problem Problems.InvalidState(string detail, string? property = null, string? typeId = null) (409) problem is returned.", null, false), new XmlParameterComment(@"ct", @"Used only to stop retrying (after the cleanup) when cancellation is requested; the between-attempts
rollback runs on its own token so the cleanup is never aborted before it runs.", null, false)], null));

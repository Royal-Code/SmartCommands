    ...
    where TContext : IWorkContext
{
    ...
    public ReativarProdutoHandler(IUnitOfWorkAccessor<TContext> accessor, IOptions<RetryOnConcurrencyOptions> retryOptions, IConcurrencyRetryProblemFactory retryProblemFactory)
    {
    ...
    public async Task<Result> HandleAsync(Guid produtoId, ReativarProduto command, CancellationToken ct)
    {
        return await this.accessor.Context.RetryOnConcurrencyAsync(
            ...
            onExhausted: () => this.retryProblemFactory.Create(command, "RoyalCode.SmartCommands.Demo.Commands.Produtos.ReativarProduto"),
            ct: ct);
    }
}

...
public class LiberarReservaEstoqueHandler<TContext> : ILiberarReservaEstoqueHandler
    where TContext : IWorkContext
{
    ...
    public LiberarReservaEstoqueHandler(IUnitOfWorkAccessor<TContext> accessor, IOptions<RetryOnConcurrencyOptions> retryOptions, IConcurrencyRetryProblemFactory retryProblemFactory, DemoDbContext db)
    {
    ...
    public async Task<Result> HandleAsync(Guid produtoId, LiberarReservaEstoque command, CancellationToken ct)
    {
        ...
        return await this.accessor.Context.RetryOnConcurrencyAsync(
            ...
            onExhausted: () => this.retryProblemFactory.Create(command, "demo.estoques.liberar"),
            ct: ct);
    }
}
